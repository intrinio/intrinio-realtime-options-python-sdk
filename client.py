...
_HEARTBEAT_MESSAGE = "{\"topic\":\"phoenix\",\"event\":\"heartbeat\",\"payload\":{},\"ref\":null}"
_HEARTBEAT_RESPONSE = "{\"topic\":\"phoenix\",\"ref\":null,\"payload\":{\"status\":\"ok\",\"response\":{}},\"event\":\"phx_reply\"}"
...
class Quote:
    ...
    def __str__(self) -> str:
        return "Quote (Type: {0}, Symbol: {1}, Price: {2:.2f}, Size: {3}, Timestamp: {4})".format(self.type, self.symbol, self.price, self.size, self.timestamp)
    def getStrikePrice(self) -> float:
        whole : int = (self.symbol[13] - '0') * 10000 + (self.symbol[14] - '0') * 1000 + (self.symbol[15] - '0') * 100 + (self.symbol[16] - '0') * 10 + (self.symbol[17] - '0')
...
class Trade:
    def __init__(self, symbol : str, price : float, size : int, totalVolume : int, timestamp : float):
        ...
        self.timestamp : float = timestamp
    def __str__(self) -> str:
        return ("Trade (Symbol: {0}, Price: {1:.2f}, Size: {2}, TotalVolume: {3}, Timestamp: {4})".format(self.symbol, self.price, self.size, self.totalVolume, self.timestamp))
    def getStrikePrice(self) -> float:
        whole : int = (self.symbol[13] - '0') * 10000 + (self.symbol[14] - '0') * 1000 + (self.symbol[15] - '0') * 100 + (self.symbol[16] - '0') * 10 + (self.symbol[17] - '0')
        part : float = (self.symbol[18] - '0') * 0.1 + (self.symbol[19] - '0') * 0.01 + (self.symbol[20] - '0') * 0.001
        return float(whole) + part
...
class OpenInterest:
    def __init__(self, symbol : str, openInterest : int, timestamp : float):
        ...
        self.timestamp : float = timestamp
    def __str__(self) -> str:
        return "OpenInterest (Symbol: {0}, Value: {1}, Timestamp: {2})".format(self.symbol, self.openInterest, self.timestamp)
...
class UnusualActivity:
    def __init__(self,
        ...
        self.timestamp : float = timestamp
    def __str__(self) -> str:
        return "Unusual Activity (Type: {0}, Sentiment: {1}, Symbol: {2}, Total Value: {3:.2f}, Total Size: {4}, Average Price: {5:.2f}, Ask at Execution: {6:.2f}, Bid at Execution: {7:.2f}, Underlying Price at Execution: {8:.2f}, Timestamp: {5})".format(self.type, self.sentiment, self.symbol, self.totalValue, self.totalSize, self.averagePrice, self.askAtExecution, self.bidAtExecution, self.priceAtExecution, self.timestamp)
    def getStrikePrice(self) -> float:
        whole : int = (self.symbol[13] - '0') * 10000 + (self.symbol[14] - '0') * 1000 + (self.symbol[15] - '0') * 100 + (self.symbol[16] - '0') * 10 + (self.symbol[17] - '0')
        part : float = (self.symbol[18] - '0') * 0.1 + (self.symbol[19] - '0') * 0.01 + (self.symbol[20] - '0') * 0.001
        return float(whole) + part
...
class _WebSocket(websocket.WebSocketApp):
    def __init__(self,
        ...
        self.__getUrl : Callable[[int,str],str] = getUrl
        self.__isFirehose : bool = isFirehose
    ...
    def __onOpen(self, ws):
        _log.info("Websocket {0} - Connected".format(self.__index))
    ...
    def __tryReconnect(self) -> bool:
        ...
        if self.isReady: return True
            ...
            with self.__wsLock: self.__isReconnecting = True
            if (time.time() - self.__lastReset > (60 * 60 * 24 * 5)):
                token : str = self.__getToken()
                super().url = self.__getUrl(self.__index, token)
            self.start()
            return False
    ...
    def __onClose(self, ws, closeStatusCode, closeMsg):
        ...
        try:
            if (not self.__isReconnecting):
                _log.info("Websocket {0} - Closed - {1}: {2}".format(self.__index, closeStatusCode, closeMsg))
                self.isReady = False
    ...
    def __onError(self, ws, error):
        _log.error("Websocket {0} - Error - {1}".format(self.__index, error))
    ...
    def __onData(self, ws, data, type, continueFlag):
        if (type == websocket.ABNF.OPCODE_BINARY):
            ...
            _log.debug("Websocket {0} - Message received".format(self.__index))
            with _txtMsgLock:
                global _txtMsgCount
                _txtMsgCount += 1
...
def _threadFn(index : int, data : queue.Queue, onTrade : Callable[[Trade], None], onQuote : Callable[[Quote], None] = None, onOpenInterest : Callable[[OpenInterest],None] = None, onUnusualActivity : Callable[[UnusualActivity],None] = None):
        ...
        try:
            ...
            for _ in range(count):
                ...
                if (msgType == 1) or (msgType == 2):
                    ...
                    if onUnusualActivity: onUnusualActivity(UnusualActivity(symbol, type, sentiment, totalValue, totalSize, averagePrice, askAtExecution, bidAtExecution, priceAtExecution, timestamp))
...
class Client:
    def __init__(self, config : Config, onTrade : Callable[[Trade], None], onQuote : Callable[[Quote], None] = None, onOpenInterest : Callable[[OpenInterest], None] = None, onUnusualActivity : Callable[[UnusualActivity],None] = None):
        if not config:
        ...
        if ((config.provider == Providers.MANUAL) or (config.provider == Providers.MANUAL_FIREHOSE)) and ((not config.manualIpAddress) or (not isinstance(config.manualIpAddress, str))):
            raise ValueError("You must specify an IP address for a manual configuration")
        if (onTrade):
            if (callable(onTrade)): self.__useOnTrade : bool = True
            else: raise ValueError("Parameter 'onTrade' must be a function")
        else: self.__useOnTrade : bool = False
        if (onQuote):
            if (callable(onQuote)): self.__useOnQuote : bool = True
            else: raise ValueError("Parameter 'onQuote' must be a function")
        else: self.__useOnQuote : bool = False
        if (onOpenInterest):
            if (callable(onOpenInterest)): self.__useOnOpenInterest : bool = True
            else: raise ValueError("Parameter 'onOpenInterest' must be a function")
        else: self.__useOnOpenInterest : bool = False
        if (onUnusualActivity):
            if (callable(onUnusualActivity)): self.__useOnUnusualActivity : bool = True
            else: raise ValueError("Parameter 'onUnusualActivity' must be a function")
        else: self.__useOnUnusualActivity : bool = False
        ...
        self.__heartbeatThread : threading.Thread = threading.Thread(None, _heartbeatFn, args=[self.__wsLock, self.__webSockets], daemon = True)
        self.__workerThreads : list[threading.Thread] = [threading.Thread(None, _threadFn, args=[i, self.__data, onTrade, onQuote, onOpenInterest, onUnusualActivity], daemon = True) for i in range(config.numThreads)]
        self.__socketThreads : list[threading.Thread] = list()
    ...
    def __trySetToken(self) -> bool:
        ...
        try:
            ...
            if response.status_code != 200:
                _log.error("Authorization Failure (status code = {0}): The authorization key you provided is likely incorrect.".format(response.status_code))
                return False
    ...
    def __socketStartFn(self, index : int, token: str):
        ...
        wsUrl : str = self.__getWebSocketUrl(token, index)
        ws : _WebSocket = _WebSocket(index,
            wsUrl, 
            self.__wsLock, 
    ...
    def start(self):
        ...
        self.__wsLock.acquire()
        try:
            wsCount : int = self.__getWebSocketCount()
            for index in range(wsCount):
                socketThread : threading.Thread = threading.Thread(None, self.__socketStartFn, args=[index, token], daemon = True)
                self.__socketThreads.append(socketThread)
        finally: self.__wsLock.release()
        for st in self.__socketThreads:
            st.start()
    ...
    def stop(self):
        ...
        for i in range(len(self.__socketThreads)):
            self.__socketThreads[i].join()
            _log.debug("Socket thread {0} joined".format(i))
        _log.info("Stopped")
